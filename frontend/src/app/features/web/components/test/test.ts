...
export class Test {
  ...
  enviarResultados() {
    const storage = localStorage.getItem('usuarioId') || sessionStorage.getItem('usuarioId');

    const idUsuario = storage ? Number(storage) :0;
    ...
    this.testService.enviarTest(paquete).subscribe({
      next: (res) => {
        const info = this.recomendaciones[res.resultadoFinal];
